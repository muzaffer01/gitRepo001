...
**Author:** Muzaffer
**Date:** 2026-08-17
**Related:** Test Plan, [Test Cases — BDD](TestCases-BDD.md) (same behaviors, Given/When/Then Gherkin format)
**Note:** Created using the Claude agent running in Windows Terminal (Claude Code CLI
on Windows 11).

Automated cases are implemented in the corresponding `*.test.jsx` / `*.spec.js` file and
executed via `npm run test` (unit/component) or `npm run test:e2e` (end-to-end). Manual
cases are executed by hand against `npm run dev`.

**TDD vs. BDD:** this document covers the imperative `describe`/`it`/`test`-style unit,
end-to-end, and manual cases — informally called "the TDD tests" in this project to
distinguish them from the declarative Given/When/Then Gherkin scenarios in the
companion [Test Cases — BDD](TestCases-BDD.md) document, which covers the **same**
behaviors (Cucumber + Playwright, `npm run test:bdd`). Neither suite is strictly
test-driven (tests were written after the implementation in both cases) — "TDD" here
just labels the imperative style as opposed to the declarative BDD one, matching how
the user referred to them.
...
| ID | Title | Steps | Expected Result | Actual Result | Type | Source | Defect |
|----|-------|-------|------------------|----------------|------|--------|--------|
| TC-FAIL-01 | Cart quantity selector respects item stock | Seed cart with a low-stock item (e.g. stock: 3), open `/cart`, inspect the quantity dropdown's options | Dropdown offers at most `min(stock, 10)` options (3, in this case) | **FAIL** — dropdown always offers exactly 10 options (1–10) regardless of stock | Automated (demo) | `demo/Cart.stockLimit.test.jsx` | [DEF-001](Defect-001-CartQuantityExceedsStock.md) |

---

**Totals:** 16 automated unit/component ("TDD") cases + 15 automated end-to-end cases,
all passing (all mapped to committed test files), 6 manual/exploratory cases for
coverage not practical to automate (TC-M02 and TC-M05 remain the only two not covered
by any automated layer), 1 known-failing case tracking an open defect (DEF-001). See
[Test Cases — BDD](TestCases-BDD.md) for the same behaviors covered in Given/When/Then
Gherkin format (21 cases, full parity except TC-M02/TC-M05).
